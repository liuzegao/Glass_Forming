...
for D_E = D_E_Initial:0.01:((N_Try)*0.01)
    ...
    Square_Delta_D_E(i_D_E) = Square_Delta_D_E(i_D_E)+(NBOratio_model(i_c)-NBO_simulation(i_c))^2+(BOratio_model(i_c)-BO_simulation(i_c))^2+(FOratio_model(i_c)-FO_simulation(i_c))^2; %Calculate the square difference
...
p(6).Color= [0,0,0.3];
p(9).Color= [0,0.7,1];
plot(i,NBO_simulation,'pr',i,BO_simulation,'pk',i,FO_simulation,'pb',...
       'LineWidth',2,...
       'MarkerSize',15,...
       'MarkerFaceColor',[0.5,0.5,0.5]);
axis([0 80 0 1]);
